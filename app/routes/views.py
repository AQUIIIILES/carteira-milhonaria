...
def get_crypto_data():
    ...
    quantities = {
        ...
        "1MBABYDOGEUSDT": {"qnt": 22008.36, "value_invest": 0.0000000023, "max_historica": 0.0000000063, "spent": 50.62},
    ...
    for symbol in cryptocurrencies:
        ...
        if response.status_code == 200:
            ...
            if name == "1MBABYDOGE":
                ...
                qnt *= 1000000.00
                price /= 1000000.00
                lucro = (price * qnt) - spent
                percentual = ((price - value_invest) / value_invest) * 100 if value_invest > 0 else 0
                price = Decimal(str(price)).normalize()
                value_invest = Decimal(str(value_invest)).normalize()
                max_historica = Decimal(str(max_historica)).normalize()
                price = format(price, '.12f')
                value_invest = format(value_invest, '.10f')
                max_historica = format(max_historica, '.10f')
...
def get_crypto_prices():
    ...
    resultado_ganho_perc = (resultado_ganho / valor_total_investido) * 100 if valor_total_investido > 0 else 0
    resultado_perda_perc = (resultado_perda / valor_total_investido) * 100 if valor_total_investido > 0 else 0
    resultado_total_perc = (resultado_total_lucro / valor_total_investido) * 100 if valor_total_investido > 0 else 0
    valor_total_investido = valor_total_investido
    valor_total_atual = valor_total_atual
    resultado_ganho = resultado_ganho
    resultado_perda = resultado_perda
    resultado_total_lucro = resultado_total_lucro
    resultado_total.append({"valor_total_investido": valor_total_investido, "valor_total_atual": valor_total_atual,
                            "resultado_ganho": resultado_ganho, "resultado_perda": resultado_perda,
                            "resultado_ganho_perc": resultado_ganho_perc, "resultado_perda_perc": resultado_perda_perc,
                            "resultado_total_lucro": resultado_total_lucro, "resultado_total_perc": resultado_total_perc})
